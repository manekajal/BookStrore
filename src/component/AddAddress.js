...
import cross from '../assets/icons/cancel3.png'
...
const initialState = {
  ...
  state : [
    ...
    { name: "Bihar", city: ["Arrah", "Begusarai", "Bhagalpur", "Chhapra", "Darbhanga", "Gaya", "Muzaffarpur", "Patna"]},
    ...
    { name: "Uttar Pradesh", city: ["Agra", "Aligarh", "Allahabad", "Bareilly", "Fatehpur", "Ghaziabad", "Hathras", "Kanpur", "Lucknow", "Noida", "Varanasi","Muzaffarnagar"]},
    { name: "Karnataka", city: ["Bengaluru","Mysore","Hassan","Davangere","Ballari","Shimoga"]},
  ],
  zip: '',
  selectedState : '',
  selectedCity: '',
...
class AddAddress extends React.Component {
  ...
  getContactById = (id) => {
    ...
    .then(response => {
      ...
      this.setContactData(responseText);
      console.log(responseText)
    }).catch(error => {
      console.log(error);
      swal("Error while fetching contact data by ID :\n" + JSON.stringify(error));
    })
  }
  ...
  checkAddress = (addressValue) => {
    if(addressValue.length === 0) {
      ...
      const PHONE_NUMBER_REGEX = RegExp("^[A-Za-z0-9-,\\.]{2,}([\\s][A-Za-z0-9-,\\.]{3,}){0,}$");
  ...
  checkValidations = async () => {
     ...
     this.checkPhoneNumber(this.state.contact);
  ...
  save = async (event) => {
    ...
    event.stopPropagation();
      ...
      if(this.state.isUpdate) {
        AddressBookService.updateContact(this.state.id,contactObject)
        .then(responseText => {
          swal("Contact Updated Successfully!!!\n");
          this.props.history.push("/home");
        }).catch(error => {
          swal("Error while updating Contact!!!\n");
        })
      } else {
        AddressBookService.addContact(contactObject)
        .then(responseDTO => {
          let responseText = responseDTO.data;
          swal("Contact Added Successfully!!!\n" );
          this.props.history.push("/home");
        }).catch(error => {
          swal("Error while adding Contact!!!\n");
        });
      }
      ...
      <div className="body">
        <header className="headerContainer header">
            <div className="logoContainer">
                <Link to="/home">
                <img src={logo} alt="" width="37px" /></Link>
                <div>
        ...
        <div className="form-content">
            <form className="form" action="#" onSubmit={this.save} onReset={this.reset}>
                ...
                <div className="row-content">
                    ...
                    <div className="validity-check">
                        <input className="input" value={this.state.contact}  onChange={this.phoneNumberChangeHandler} type="tel" id="tel" name="tel" />
                        <valid-message className="valid-tel" htmlFor="tel">{this.state.valid.contact}</valid-message>
                        <error-output className="tel-error" htmlFor="tel">{this.state.error.contact}</error-output>
                    </div>
                </div>
                <div className="row-content">
